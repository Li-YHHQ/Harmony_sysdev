...
static int g_aht20_available = 1;  // 1=可用 0=不可用
...
void i2c_aht20_init(){
    ...
    // 校准AHT20，如果校准失败，等待100ms后重试，最多重试3次
    int retry = 0;
    while (IOT_SUCCESS != AHT20_Calibrate() && retry < 3) {
        printf("AHT20 sensor init failed! retry=%d\r\n", retry);
        usleep(100 * 1000);
        retry++;
    }
    if (retry >= 3) {
        printf("AHT20 init skipped, sensor may be damaged!\r\n");
        g_aht20_available = 0;
    }
}
...
int aht20_getDat(float *temp, float *humi){
    if (!g_aht20_available) {
        printf("AHT20 not available, using last value\r\n");
        return 0;  // 返回成功但保持上次值不变
    }
