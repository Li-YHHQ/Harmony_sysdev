...
static uint32_t ssd1306_SendData(uint8_t* data, size_t size)
{
    ...
    uint32_t retval = IoTI2cWrite(id, (SSD1306_I2C_ADDR << 1) | 0, i2cData.sendBuf, i2cData.sendLen);
    if (retval != 0) {
...
void ssd1306_DrawChinese(uint8_t n, FontDef Font, SSD1306_COLOR color) {
    uint32_t i, j;
    uint8_t b;
    for (i = 0; i < Font.FontWidth * 2; i++) {
        b = Font.data1[n * Font.FontWidth * 2 + i];
        for (j = 0; j < 8; j++) {
            if ((b >> j) & 0x01) {
                if (i < Font.FontWidth) {
                    ssd1306_DrawPixel(SSD1306.CurrentX + i,
                        SSD1306.CurrentY + j, color);
                } else {
                    ssd1306_DrawPixel(SSD1306.CurrentX + (i - Font.FontWidth),
                        SSD1306.CurrentY + 8 + j, color);
                }
            }
        }
    }
...
void ssd1306_DrawImage(const unsigned char *img, uint8_t x, uint8_t y, uint8_t w, uint8_t h) {
    uint16_t i, j;
    uint8_t byte, bit;
    for (j = 0; j < h; j++) {
        for (i = 0; i < w; i++) {
